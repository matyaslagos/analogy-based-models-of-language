...
for run in results_by_run:
    ...
    for sample_size in results_by_run[run]:
        ...
        for function in results_by_run[run][sample_size]:
            ...
            if function != "sample_freqs" and function != "min_conf":
...
# Checking that ndcg_store is populated correctly for run 32, sample size 50, and function "skew_div":
found_words = results_by_run[32][50]["skew_div"]
sample_total_freq = sum(results_by_run[32][50]["sample_freqs"])
print(ndcg_k_min(found_words, sample_total_freq))

print(ndcg_store[32][50]["skew_div"])

...
"""
Comprehensive tests for the radix trie implementation.

This test suite compares the radix trie against the original trie implementation
to ensure functional equivalence, and also includes additional unit tests.
"""
...
def test_basic_insertion():
    ...
    # Create both trie types
    standard_trie = syn.FreqTrie()
    radix_trie_obj = radix.RadixFreqTrie()

    # Insert a simple sequence
    sequence = ('<', 'the', 'cat', '>')
    standard_trie._insert(sequence)
    radix_trie_obj._insert(sequence)
    ...
    for seq in test_cases:
        ...
        rad_freq = radix_trie_obj.freq(seq)
        match = "✓" if std_freq == rad_freq else "✗"
        if std_freq != rad_freq:
            all_passed = False
        print(f"{match} freq({seq}): standard={std_freq}, radix={rad_freq}")
...
def test_small_corpus():
    ...
    standard_trie.setup('corpora/small_test_corpus.txt')
    radix_trie_obj.setup('corpora/small_test_corpus.txt')
...
def test_right_neighbors():
    ...
    rad_neighbors = sorted(list(radix_trie_obj.right_neighbors(test_seq)))

    print(f"Standard trie: {std_neighbors}")
    print(f"Radix trie:    {rad_neighbors}")

    all_passed = std_neighbors == rad_neighbors
...
def test_left_neighbors():
    ...
    rad_neighbors = sorted(list(radix_trie_obj.left_neighbors(test_seq)))
...
def test_shared_right_neighbors():
    ...
    rad_shared = sorted(list(radix_trie_obj.shared_right_neighbors(seq1, seq2)))

    print(f"Standard trie: {std_shared}")
    print(f"Radix trie:    {rad_shared}")

    all_passed = std_shared == rad_shared
...
def test_shared_left_neighbors():
    ...
    rad_shared = sorted(list(radix_trie_obj.shared_left_neighbors(seq1, seq2)))
...
def test_edge_compression():
    ...
    # Start from root and check if paths are compressed
    node = radix_trie_obj.fw_root
    if '<' in node.children:
        child = node.children['<']
...
def test_neighbor_length_constraints():
    ...
    rad_neighbors = sorted(list(radix_trie_obj.right_neighbors(base_seq, min_length=2, max_length=3)))

    print(f"Standard: {std_neighbors}")
    print(f"Radix:    {rad_neighbors}")
...
def run_all_tests():
    """Run all test cases."""
    print("=" * 60)
    print("RADIX TRIE IMPLEMENTATION TEST SUITE")
    print("=" * 60)

...
Task: write function `concordance_by_label()` for returning list of `[left_context, sequence, right_context]` triples where `sequence` is a word sequence that was found in a sentence labelled with input label, and `left_context` and `right_context` are its full left and right contexts in the sentence. Should work like this:
```python
